					...
					<tbody>
						{latestExpenses.map((item, index) => (
							<tr key={index}>
								<td>
									<div
										className={`${css.status} ${item.type === "Income" ? css.income : item.type !== "Error" ? css.expense : css.error}`}
									>
										{item.type}
									</div>
								</td>
								...
								<td>
									<p
										className={`${css.amount} ${item.type === "Income" ? "" : item.type !== "Error" ? css.negative : css.absent}`}
									>
										{item.amount}
									</p>
								</td>
