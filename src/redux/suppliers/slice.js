...
import { getSuppliers, addSupplier, updateSupplier, deleteSupplier } from "./operations";
    ...
    builder
      ...
      .addCase(addSupplier.fulfilled, (state, action) => {
        ...
        if (state.pagination.page === 1) {
          state.items = [action.payload.data, ...state.items.slice(0, state.pagination.perPage - 1)];
        }
      })
      ...
      .addCase(updateSupplier.fulfilled, (state, action) => {
        ...
        const index = state.items.findIndex(item => item._id === updatedSupplier._id);
        if (index !== -1) {
